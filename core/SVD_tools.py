...
def m_svd(mrange, n, k, n_it, er_out=True, random=False):
    n_m = np.size(mrange)
...
def n_svd(m, nrange, k, n_it, er_out=True, random=False):
    n_n = np.size(nrange)
...
def k_svd(m, n, krange, n_it, er_out=True, random=False):
    n_k = np.size(krange)
...
def run_plot(type1, type2):
    ...
    # plot
    plot_svd(range1, range2, error1, t1, error2, t2, t_rp, type1, type2)
